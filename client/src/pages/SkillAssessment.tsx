...
const SkillAssessmentPage: React.FC = () => {
  ...
  const fetchSkills = async (studentId: number) => {
    try {
      const response = await axios.get(
        `http://localhost:5008/api/skills/${studentId}`
      );
  ...
  const getIndividualSkills = () => {
    ...
    return {
      technicalSkills:
        userSkills.technical?.flatMap(
          (item) => item.skill?.split(",").map((s) => s.trim()) || []
        ) || [],
      languageSkills:
        userSkills.languages?.flatMap(
          (item) => item.language?.split(",").map((s) => s.trim()) || []
        ) || [],
    };
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <header className="flex items-center justify-between mb-8">
        ...
        {activeAssessment && (
          <Button onClick={() => setActiveAssessment(null)}>Back</Button>
        )}
      </header>
      ...
      {activeAssessment ? (
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {activeAssessment} - Question {currentQuestionIndex + 1} of{" "}
                {questions.length}
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              {availableAssessments.map((skill) => (
                <div
                  key={skill}
                  className="border rounded-lg p-4 hover:shadow-md mb-4"
                >
